...
def health(db_path: Path, stale_after: int = DEFAULT_STALE_SEC) -> dict:
    ...
    try:
        ...
        pairs, total, newest = conn.execute(
            f"SELECT COUNT(DISTINCT pair), COUNT(*), MAX({epoch_sql}) FROM ticks"
        ).fetchone()
        ...
        stalled = [
            ...
            for row in conn.execute(
                f"SELECT pair, MAX({epoch_sql}) FROM ticks GROUP BY pair HAVING ? - MAX({epoch_sql}) > ?",
                (now, stale_after),
            )
        ]
        report["stalled_pairs"] = stalled
        # Every pair stalling is already covered by the last-tick check above;
        # naming them individually matters when only some went quiet.
